...
class JsonRpcResponseDeserializer implements JsonDeserializer<Response<?>> {
    ...
    public Response<?> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
        ...
        if (!(json instanceof JsonObject)) {
            throw new JsonParseException("JsonObject expected, found " + json.getClass().getSimpleName());
        }
        ...
        if (jObject.has(CHANEL_PROPERTY) && jObject.has(DATA_PROPERTY)) {
            return new Response<>(null, context.deserialize(jObject.get(DATA_PROPERTY),
        ...
        if (idAsJsonElement != null && !(idAsJsonElement instanceof JsonNull)) {
            try {
        ...
        if (jObject.has(ERROR_PROPERTY)) {
            ...
            if (jObject.has(RESULT_PROPERTY)) {
                ...
                return new Response<>(id, context.deserialize(jObject.get(RESULT_PROPERTY), parameterizedType.getActualTypeArguments()[0]));
